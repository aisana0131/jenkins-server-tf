...
resource "aws_instance" "web" {
  ...
  user_data = <<-EOF
              ...
              EOF
  tags = {
    Name = "FullAccess-${var.env}"  // string interpolation 
    env = var.env
  }
}
...
resource "aws_iam_role" "s3_full_access_role_jenkins" {
...
resource "aws_iam_role_policy_attachment" "s3_full_access_attachment" {
  role       = aws_iam_role.s3_full_access_role_jenkins.name
